...
/** Namespaced storage key for badge persistence */
const STORAGE_KEY = "tf:dismissedBadges:v1";
...
export default function Sidebar({ darkMode }) {
  ...
  // STORE WHICH BADGES THE USER HAS DISMISSED (PERSISTED TO LOCALSTORAGE)
  const [dismissedBadgeMap, setDismissedBadgeMap] = useState(() => readBadgeMap());
  ...
  // WHEN BADGE MAP CHANGES: PERSIST TO LOCALSTORAGE (background sync)
  useEffect(() => {
    writeBadgeMap(dismissedBadgeMap);
  }, [dismissedBadgeMap]);
  ...
  // RECORD THAT A SPECIFIC BADGE SHOULD NO LONGER BE SHOWN (immediate write-through)
  const recordBadgeDismissalForKey = (badgeKey) => {
    if (!badgeKey) return;
    setDismissedBadgeMap((previous) => {
      const next = { ...previous, [badgeKey]: true };
      writeBadgeMap(next); // ensure persistence even if component unmounts during navigation
      return next;
    });
  };
      ...
      {activeNavigationItems.map(({ to, label, icon: Icon, end, badge, badgeKey }) => {
        ...
        return (
          <NavLink
            ...
            onClick={() => {
              // WHEN A BADGED ITEM IS CLICKED, MARK ITS BADGE AS DISMISSED (write-through)
              if (badgeKey) recordBadgeDismissalForKey(badgeKey);
              // OPTIONAL CALLBACK FOR CLOSING THE MOBILE DRAWER
              if (onNavItemChosen) onNavItemChosen();
            }}
