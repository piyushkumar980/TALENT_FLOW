          ...
          {filteredInterviewEntries.length === 0 ? (
            ...
            filteredInterviewEntries.map((entry) => {
              ...
              return (
                ...
                >
                  {/* ✅ Responsive polish for <488px: stack details + make actions full-width */}
                  <div className="flex items-start justify-between gap-4 max-[488px]:flex-col">
                    {/* LEFT COLUMN: ICON + PRIMARY DETAILS */}
                    <div className="flex items-start gap-4 min-w-0">
                      <div
                        className={`w-12 h-12 rounded-xl grid place-items-center ${
                      ...
                      <div className="min-w-0">
                        ...
                        {/* Company • Date -> stack on tiny screens */}
                        <div
                          className={`mt-1 flex flex-wrap items-center gap-2 max-[488px]:flex-col max-[488px]:items-start max-[488px]:gap-1 ${
                            darkMode ? "text-slate-300" : "text-slate-700"
                          }`}
                        >
                          ...
                          <span className="max-[488px]:hidden">•</span>
                          <span className="inline-flex items-center gap-1">
                            <Calendar size={14} />
                            {formatInterviewDateTimeForList(entry.when)}
                          </span>
                        ...
                        {/* Mode/Location • Contact -> stack on tiny screens */}
                        <div
                          className={`mt-1 flex flex-wrap items-center gap-3 max-[488px]:flex-col max-[488px]:items-start max-[488px]:gap-1 ${
                            darkMode ? "text-slate-400" : "text-slate-600"
                          }`}
                        >
                          {entry.mode === "Video" && (
                    ...
                    {/* RIGHT COLUMN: CONTEXTUAL ACTIONS */}
                    <div className="flex-shrink-0 flex flex-col sm:flex-row items-stretch gap-2 max-[488px]:w-full">
                      {/* JOIN LINK SHOWS ONLY WHEN THE INTERVIEW IS IN THE FUTURE AND HAS A LINK/MODE */}
                      {isFuture && (entry.mode === "Video" || entry.link) && (
                        <a
                          href={entry.link || "#"}
                          className={`inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition max-[488px]:w-full ${
                            darkMode
                              ? "bg-indigo-600 text-white hover:bg-indigo-500"
                              : "bg-indigo-600 text-white hover:bg-indigo-700"
                          }`}
