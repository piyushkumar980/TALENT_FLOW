...
function useRecruiterHeaderCopy() {
  ...
  if (pathname.startsWith("/hr/jobs/new"))
    return { title: "Create Job", subtitle: "This is my platform where I'm creating new job opportunities to attract top talent." };

  if (pathname.startsWith("/hr/jobs"))
    return { title: "Jobs", subtitle: "Organize your open positions and streamline hiring." };

  if (pathname.startsWith("/hr/candidates"))
    return { title: "Candidates", subtitle: "View, track, and shortlist applicants across all jobs." };

  if (pathname.startsWith("/hr/assessments"))
    return { title: "Assessments", subtitle: "Create and review assessments to identify the best fit." };

  if (pathname.startsWith("/hr/dashboard/recent-jobs"))
    return { title: "Latest Job Postings", subtitle: "Track the most recent openings and monitor their performance in real time." };

  return { title: "Dashboard", subtitle: "Welcome back! Here's what's happening with your hiring pipeline." };
}
...
function useJobseekerHeaderCopy() {
  ...
  if (pathname.startsWith("/jobseeker/find-jobs"))
    return { title: "Find Jobs", subtitle: "Browse openings that match your skills and interests." };

  if (pathname.startsWith("/jobseeker/apply/"))
    return { title: "Apply to Job", subtitle: "Review the job details and submit your best application." };

  if (pathname.startsWith("/jobseeker/assessments"))
    return { title: "Assessments", subtitle: "Take and review assessments to stand out." };

  if (pathname.startsWith("/jobseeker/applications"))
    return { title: "My Applications", subtitle: "Track statuses, timelines, and next steps." };

  if (pathname.startsWith("/jobseeker/saved"))
    return { title: "Saved Jobs", subtitle: "Revisit roles you’ve bookmarked to apply later." };

  if (pathname.startsWith("/jobseeker/interviews"))
    return { title: "Interviews", subtitle: "Track upcoming calls and past rounds." };

  if (pathname.startsWith("/jobseeker/messages"))
    return { title: "Messages", subtitle: "Conversations and updates from recruiters." };

  return { title: "Welcome back!", subtitle: "Here’s what’s happening with your job search today." };
}
...
export default function App() {
  ...
  // refs for robust outside-click handling
  ...
  const handleDarkModeToggle = () => setIsDarkThemeEnabled((previous) => !previous);

  const handleNotificationsToggle = (e) => {
    // prevent immediate outside-click from closing when opening
    e?.stopPropagation?.();
    setAreNotificationsVisible((open) => !open);
    if (!areNotificationsVisible && unreadNotificationCount > 0) {
      setUnreadNotificationCount(0);
  ...
  // robust outside click using composedPath + capture phase
  useEffect(() => {
    function onDocClick(e) {
      if (!areNotificationsVisible) return;
      const path = (e.composedPath && e.composedPath()) || [];
      const clickedInsidePanel = panelRef.current && path.includes(panelRef.current);
      const clickedBell = bellRef.current && path.includes(bellRef.current);
      if (!clickedInsidePanel && !clickedBell) setAreNotificationsVisible(false);
    }
    document.addEventListener("click", onDocClick, true);
    return () => document.removeEventListener("click", onDocClick, true);
  }, [areNotificationsVisible]);

  // sample notifications (dummy data)
  const recruiterNotifications = [
    { id: "hr-1", type: "application", title: "New Job Application", message: "Sarah Johnson applied for Senior UX Designer position", time: "10 minutes ago", unread: true, priority: "high" },
    { id: "hr-2", type: "assessment", title: "Assessment Completed", message: "Michael Thompson completed the Frontend Developer assessment", time: "2 hours ago", unread: true, priority: "medium" },
    { id: "hr-3", type: "interview", title: "Interview Scheduled", message: "Interview with Jessica Williams for Product Manager role at 2:30 PM tomorrow", time: "5 hours ago", unread: true, priority: "high" },
  ...
  const activeRoleNotifications = isJobseekerRoute ? jobseekerNotifications : recruiterNotifications;
  ...
  }, [location.pathname]); // recompute when route changes between role areas
  ...
  const NotificationBellButton = (
    <button
      ...
      onClick={handleNotificationsToggle}
      className={`relative p-2 rounded-lg ${
        isDarkThemeEnabled ? "text-slate-400 hover:bg-slate-700" : "text-slate-500 hover:bg-slate-100"
      }`}
      aria-label="Open notifications"
    >
      <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const HeaderRecruiter = (
    <header
      className={`sticky top-0 z-30 ${
        isDarkThemeEnabled ? "bg-slate-900/80 border-slate-800" : "bg-white/80 border-slate-200"
  ...
  const HeaderJobseeker = (
    <header
      className={`sticky top-0 z-30 ${
        isDarkThemeEnabled ? "bg-slate-900/90 border-slate-800" : "bg-white/90 border-slate-200"
      } backdrop-blur border-b shadow-sm`}
    >
      <div className="max-w-6xl mx-auto flex items-center gap-3 pl-16 pr-4 py-5 lg:px-6">
        <div className="flex-1">
  ...
  return (
    <div className={`min-h-screen ${isDarkThemeEnabled ? "bg-slate-900" : "bg-slate-50"}`}>
      ...
      <div className="lg:pl-64 min-h-screen">
        {isJobseekerRoute ? HeaderJobseeker : HeaderRecruiter}

        {/* NOTIFICATIONS PANEL VIA PORTAL (always above everything) */}
        {areNotificationsVisible &&
          createPortal(
            <div className="fixed inset-0 z-[9999] bg-black/30 backdrop-blur-sm flex justify-center items-start pt-20 px-4">
              <div
                ref={panelRef}
              ...
              >
                <div className={`p-4 border-b ${isDarkThemeEnabled ? "border-slate-700" : "border-slate-200"} flex justify-between items-center`}>
                  <div>
                    <h3 className={`font-bold text-lg ${isDarkThemeEnabled ? "text-white" : "text-slate-800"}`}>Notifications</h3>
                            ...
                            {note.priority === "high" && (
                              <span
                                className={`px-1.5 py-0.5 text-xs font-medium rounded-full ${
                                  isDarkThemeEnabled ? "bg-red-900/30 text-red-300" : "bg-red-100 text-red-800"
                                }`}
                              >
                                Important
                              </span>
